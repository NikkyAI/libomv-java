...
public class MultiMap<K, V> {
	...
	/**
	 * Creates a MultiMap which uses a HashMap as the underlying map and ArrayLists
	 * to store the values.
	 */
	public MultiMap() {
	...
	/**
	 * Creates a MultiMap which allows the programmer to specify the underlying map
	 * and uses ArrayLists to store the values.
	 *
	 * @param innerMap
	 */
	 ...
	 *
	 * @param key
	 * @return a Collection containing each of the values associated with the key,
	 *         or null if no values are associated with this key.
	 */
	 ...
	 * @return a List containing each of the values associated with the key, or null
	 *         if no values are associated with this key.
	 */
	 ...
	 * synonymous with the valueCount() method and was included for consistency with
	...
	/**
	 * Associates the supplied value with the supplied key. MultiMaps allow multiple
	 * values per key but duplicates are only allowed if the underlying Collection
	 * supplied to the constructor also allows them. The default ArrayList does
	 * allow duplicates.
	 *
	 * @param key
	 * @param value
	 ...
	 *
	 * @param key
	 * @return a collections containing the values previously associated with the
	 *         key, or null if no values were previously associated with it.
	 */
	 ...
	 * <br>
	 * Note: If the key was previously added to the MultiMap and all of its values
	 * have subsequently been removed, this method will return false.
	 *
	 * @param key
	...
	/**
	 * Retrieves all the values contained in the MultiMap. This method is supplied
	 * as a convenience but is likely to have poor performance.
	 *
	...
	/**
	 * A standard Map-like entry set. Each key is mapped to a collection containing
	 * all its values.
	 *
	...
	/**
	 * Returns an entry set where each key is mapped to one value. As a consequence
	 * of this, each key may appear multiple times in the entry set. This method is
	 * supplied as a convenience and may have poor performance.
	 *
	 * @return
	 */
	...
	public String toString() {
		...
		for (Map.Entry<K, List<V>> e : entrySet()) {
			string = string + " " + e.getKey().toString() + ", count: " + e.getValue().size() + " "
					+ e.getValue().toString() + "\n";
		}
		return string + ")";
	}
