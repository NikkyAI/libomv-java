...
/**
 * This decodes maps which are defined in the sYCC colorspace into the sRGB
 * colorspadce.
 * 
...
public class SYccColorSpaceMapper extends ColorSpaceMapper
{
	...
	private static DataBlkFloat[] mult(DataBlkFloat[] inblk)
	{
		...
		for (j = 0; j < length; ++j)
		{
			out[0][j] = (Matrix00 * in[0][inblk[0].offset + j] + Matrix01 * in[1][inblk[1].offset + j] + Matrix02
					* in[2][inblk[2].offset + j]);

			out[1][j] = (Matrix10 * in[0][inblk[0].offset + j] + Matrix11 * in[1][inblk[1].offset + j] + Matrix12
					* in[2][inblk[2].offset + j]);

			out[2][j] = (Matrix20 * in[0][inblk[0].offset + j] + Matrix21 * in[1][inblk[1].offset + j] + Matrix22
					* in[2][inblk[2].offset + j]);
		}
	...
	private static DataBlkInt[] mult(DataBlkInt[] inblk)
	{
		...
		for (j = 0; j < length; ++j)
		{
			out[0][j] = (int) (Matrix00 * in[0][inblk[0].offset + j] + Matrix01 * in[1][inblk[1].offset + j] + Matrix02
					* in[2][inblk[2].offset + j]);

			out[1][j] = (int) (Matrix10 * in[0][inblk[0].offset + j] + Matrix11 * in[1][inblk[1].offset + j] + Matrix12
					* in[2][inblk[2].offset + j]);

			out[2][j] = (int) (Matrix20 * in[0][inblk[0].offset + j] + Matrix21 * in[1][inblk[1].offset + j] + Matrix22
					* in[2][inblk[2].offset + j]);
		}
	...
	public String toString()
	{
		...
		for (i = 0; i < ncomps; ++i)
		{
			rep_comps.append("  ").append("component[").append(String.valueOf(i)).append("] height, width = (")
					.append(src.getCompImgHeight(i)).append(", ").append(src.getCompImgWidth(i)).append(")")
					.append(eol);
		}
