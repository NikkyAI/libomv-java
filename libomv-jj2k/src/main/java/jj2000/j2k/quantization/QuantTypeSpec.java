		...
		while (stk.hasMoreTokens())
		{
			...
			switch (word.charAt(0))
			{
				...
				case 'e': // expounded quantization step size specification
					if (!word.equalsIgnoreCase("reversible") && !word.equalsIgnoreCase("derived")
					...
					if (pl.getBooleanParameter("lossless")
							&& (word.equalsIgnoreCase("derived") || word.equalsIgnoreCase("expounded")))
					{
						throw new IllegalArgumentException("Cannot use non reversible quantization with "
								+ "'-lossless' option");
					}
			...
			else
			{
				...
				switch (specValType[0][0])
				{
					...
					case SPEC_TILE_COMP:
						...
						break;
					default:
						throw new IllegalArgumentException("unhandled spec tile type " + specValType[0][0] + " in quantization");
				}
			}
		}
	}
