...
/**
 * This abstract class provides the general interface for quantizers. The input
 * of a quantizer is the output of a wavelet transform. The output of the
 * quantizer is the set of quantized wavelet coefficients represented in
 * sign-magnitude notation (see below).
 * 
 * <p>
 * This class provides default implementation for most of the methods (wherever
 * it makes sense), under the assumption that the image, component dimensions,
 * and the tiles, are not modifed by the quantizer. If it is not the case for a
 * particular implementation, then the methods should be overriden.
 * 
 * <p>
 ...
 * the next M most significat bits. The rest of the bits (least significant
 ...
 * 
 * <p>
 * NOTE: At the moment only quantizers that implement the 'CBlkQuantDataSrcEnc'
...
public abstract class Quantizer extends ImgDataAdapter implements CBlkQuantDataSrcEnc
{
	...
	private final static String[][] pinfo = {
			{
					...
					"Specifies which quantization type to use for specified "
							...
							+ "<id>: Supported quantization types specification are : 'reversible' "
							+ "(no quantization), 'derived' (derived quantization step size) and 'expounded'.\n"
	 ...
	 * Quantizer. The 'stepWMSE' field is calculated for each subband which is a
	 * leaf in the tree rooted at 'sb', for the specified component. The subband
	 * tree 'sb' must be the one for the component 'n'.
	 * 
	 * @param sb
	 ...
	 * 
	 * <P>
	 * This method gets the subband tree from the source and then calculates the
	 * magnitude bits for each leaf using the method calcSbParams().
	 * 
	 * @param t
	 ...
	 * specified in the options in the parameter list 'pl', and having 'src' as
	 * the source of data to be quantized. The 'rev' flag indicates if the
