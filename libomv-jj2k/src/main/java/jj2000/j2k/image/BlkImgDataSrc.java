...
/**
 * This interface defines the methods to transfer image data in blocks, without
 * following any particular order (random access). This interface does not
 * define the methods to access the image characteristics, such as width,
 * height, number of components, tiles, etc., or to change the current tile.
 * That is provided by other interfaces such as ImgData.
 * 
 * <P>
 * This interface has the notion of a current tile. All data, coordinates and
 * dimensions are always relative to the current tile. If there is only one tile
 * then it is equivalent as having no tiles.
 * 
 * <P>
 * A block of requested data may never cross tile boundaries. This should be
 * enforced by the implementing class, or the source of image data.
 * 
 * <P>
 * This interface defines the methods that can be used to retrieve image data.
 * Implementing classes need not buffer all the image data, they can ask their
...
public interface BlkImgDataSrc extends ImgData
{
	 ...
	 * least significant integral (i.e. non-fractional) bit, which is equivalent
	 * to the number of fractional bits. For instance, for fixed-point values
	 * with 2 fractional bits, 2 is returned. For floating-point data this value
