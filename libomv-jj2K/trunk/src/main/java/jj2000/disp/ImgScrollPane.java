...
public class ImgScrollPane extends Container
{
	...
	public final static int SCROLLBARS_AS_NEEDED = ScrollPane.SCROLLBARS_AS_NEEDED;
	...
	public synchronized void setZoom(float zf)
	{
		...
		if (sbType == SCROLLBARS_AS_NEEDED)
			doLayout();
	...
	public synchronized void setScrollPosition(int x, int y)
	{
		...
		if (imgDisplay.lastUpdateOffset != null && imgDisplay.lastUpdateOffset.x == x
		...
		if (sbType == SCROLLBARS_AS_NEEDED && imgDisplay.calcDim())
		{
			...
			if (sz.width >= imsz.width + 2 * INTERNAL_GAP)
			{
				...
				else
				{
					...
					// horizontal.
					vsbar.setVisible(true);
					if (sz.width >= imsz.width + 3 * INTERNAL_GAP + SCROLLBAR_THICKNESS)
					{
						hsbar.setVisible(false);
					}
					else
			...
			else
			{
				...
				// one.
				hsbar.setVisible(true);
				if (sz.height >= imsz.height + 3 * INTERNAL_GAP + SCROLLBAR_THICKNESS)
				{
					vsbar.setVisible(false);
				}
				else
	...
	private void setScrollbars()
	{
		...
		if (lastZoom == 0f)
			lastZoom = zoom;
		if (lastSize == null)
			lastSize = new Dimension(asz.width, asz.height);
	...
	private class ImageScrollDisplay extends Canvas
	{
		...
		public void update(Graphics g)
		{
			...
			synchronized (ImgScrollPane.this)
			{
				...
				if (lastUpdateOffset != null && (clip.width < b.width || clip.height < b.height))
				{
			...
			if ((status & ImageObserver.ALLBITS) == 0 && (loffx != offx || loffy != offy))
			{
				erase = true;
			}
			...
			if (copyScroll && !erase && (loffx != offx || loffy != offy) && (status & ImageObserver.ALLBITS) != 0)
			{
	...
	class ISPScrollbar extends Scrollbar
	{
		 ...
		 * 
		 * @param svt
		 *            The scrollbar visible type
		 */
		ISPScrollbar(int orientation, int value, int visible, int min, int max)
		{
