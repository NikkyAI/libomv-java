...
public class BlkImgDataSrcImageProducer implements ImageProducer
{
	...
	public void startProduction(ImageConsumer ic)
	{
		...
		for (i = cons.length - 1; i >= 0; i--)
		{
			...
			cons[i].setDimensions(src.getCompImgWidth(0), src.getCompImgHeight(0));
		...
		for (int y = 0; y < nT.y; y++)
		{
			...
			for (int x = 0; x < nT.x; x++, tIdx++)
			{
				...
				// dimensions.
				tOffx = src.getCompULX(0) - (int) Math.ceil(src.getImgULX() / (double) src.getCompSubsX(0));
				tOffy = src.getCompULY(0) - (int) Math.ceil(src.getImgULY() / (double) src.getCompSubsY(0));
