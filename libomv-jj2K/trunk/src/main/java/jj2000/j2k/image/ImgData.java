 ...
 * number of components, etc.). The image can be tiled or not (i.e. if the image
 * is not tiled then there is only 1 tile). It should be implemented by all
 * classes that provide image data, such as image file readers, color
 * transforms, wavelet transforms, etc. This interface, however, does not define
 * methods to transfer image data (i.e. pixel data), that is defined by other
 * interfaces, such as 'BlkImgDataSrc'.
 * 
 * @see BlkImgDataSrc
 */
public interface ImgData
{
	 ...
	 */
	public int setTile(int x, int y) throws IllegalArgumentException;
	 ...
	 * @return The new tile index
	 */
	public int nextTile() throws NoNextElementException;
