...
public class ImgWriterPPM extends ImgWriter
{
	...
	public void write(int ulx, int uly, int w, int h) throws IOException
	{
		...
		tOffx = src.getCompULX(cps[0]) - (int) Math.ceil(src.getImgULX() / (double) src.getCompSubsX(cps[0]));
		tOffy = src.getCompULY(cps[0]) - (int) Math.ceil(src.getImgULY() / (double) src.getCompSubsY(cps[0]));
