 ...
 * Specialized abstract classes that work on particular data types (int, float)
 ...
 * 
 * <ul>
 * <li>The first sample to filter is the low-pass one. As a consequence, if the
 * input signal is of odd-length then the low-pass output signal is one sample
 * longer than the high-pass output one. Therefore, if the length of input
 * signal is N, the low-pass output signal is of length N/2 if N is even and
 * N/2+1/2 if N is odd, while the high-pass output signal is of length N/2 if N
 * is even and N/2-1/2 if N is odd.</li>
 * 
 * <li>The normalization is 1 for the DC gain and 2 for the Nyquist gain (Type I
 * normalization), for both reversible and non-reversible filters.</li>
 * 
 ...
 * 
 * <li>The analyze method may seem very complicated, but is designed to minimize
 ...
 * applicable to full-frame transforms as well.</li>
 * 
 * <li>All filters should implement the equals() method of the Object class. The
 * call x.equals(y) should test if the 'x' and 'y' filters are the same or not,
 * in what concerns the bit stream header syntax (two filters are the same if
 * the same filter code should be output to the bit stream).</li>
 * </ul>
 * 
 * @see AnWTFilterInt
 * @see AnWTFilterFloat
...
public abstract class AnWTFilter implements WaveletFilter
{
	 ...
	 * array, and this is determined by the inStep argument. This means that the
	 * first sample of the input signal is inSig[inOff], the second is
	 * inSig[inOff+inStep], the third is inSig[inOff+2*inStep], and so on.
	 * Therefore if inStep is 1 there is no interleaving. This feature allows to
	 ...
	 * the input signal using an overlap and thus producing the same
	 * coefficients at the output. The tailOvrlp argument specifies how many
	 * samples in the input signal, before the first one to be filtered, can be
	 * used for overlap. Then, the filter instead of extending the input signal
	 * will use those samples to calculate the first output samples. The
	 * argument tailOvrlp can be 0 for no overlap, or some value that provides
	 * partial or full overlap. There should be enough samples in the input
	 * signal, before the first sample to be filtered, to support the overlap.
	 * The headOvrlp provides the same functionality but at the end of the input
	 * signal. The inStep argument also applies to samples used for overlap.
	 * This overlap feature can be used for line-based wavelet transforms (in
	 ...
	 *            of the correct type (e.g., it must be int[] if getDataType()
	 *            returns TYPE_INT).
	 * 
	 * @param inOff
	 ...
	 * 
	 * @param inStep
	 *            This is the step, or interleave factor, of the input signal
	 *            samples in the inSig array. See above.
	 * 
	 * @param tailOvrlp
	 *            This is the number of samples in the input signal before the
	 *            first sample to filter that can be used for overlap. See
	 *            above.
	 * 
	 * @param headOvrlp
	 *            This is the number of samples in the input signal after the
	 *            last sample to filter that can be used for overlap. See above.
	 * 
	 * @param lowSig
	 *            This is the array where the low-pass output signal is placed.
	 *            It must be of the same type as inSig and it should be long
	 *            enough to contain the output signal.
	 * 
	 * @param lowOff
	 *            This is the index in lowSig of the element where to put the
	 *            first low-pass output sample.
	 * 
	 * @param lowStep
	 *            This is the step, or interleave factor, of the low-pass output
	 *            samples in the lowSig array. See above.
	 * 
	 * @param highSig
	 ...
	 * 
	 * @param highOff
	 *            This is the index in highSig of the element where to put the
	 *            first high-pass output sample.
	 * 
	 * @param highStep
	 ...
	 * which is the low-pass filter. This is the time-reversed impulse response
	 * of the low-pass synthesis filter. It is used to calculate the L2-norm of
	 ...
	 * which is the high-pass filter. This is the time-reversed impulse response
	 * of the high-pass synthesis filter. It is used to calculate the L2-norm of
	 * the synthesis basis functions for a particular subband (also called
	 * energy weight).
	 * 
	 * <p>
	 ...
	 * filters, given the syhthesis waveform of the previous stage. This is the
	 * result of upsampling 'in' by 2, and concolving it with the low-pass
	 * synthesis waveform of the filter. The length of the returned signal is
	 * 2*in_l+lp_l-2, where in_l is the length of 'in' and 'lp_l' is the lengthg
	 * of the low-pass synthesis filter.
	 * 
	 * <p>
	 ...
	 * 
	 * @param out
	 *            If non-null this array is used to store the resulting signal.
	 *            It must be long enough, or an IndexOutOfBoundsException is
	 *            thrown.
	 * 
	 ...
	 * filters, given the syhthesis waveform of the previous stage. This is the
	 * result of upsampling 'in' by 2, and concolving it with the high-pass
	 * synthesis waveform of the filter. The length of the returned signal is
	 * 2*in_l+hp_l-2, where in_l is the length of 'in' and 'hp_l' is the lengthg
	 * of the high-pass synthesis filter.
	 * 
	 * <p>
