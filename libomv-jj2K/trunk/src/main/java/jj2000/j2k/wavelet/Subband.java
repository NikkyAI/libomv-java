...
public abstract class Subband
{
	 ...
	 * gain of a subband is defined as the gain of the previous subband (i.e.
	 * the one from which this one was obtained) multiplied by the line gain and
	 * by the column gain. The line (column) gain is the gain of the line
	 * (column) filter that was used to obtain it, which is the DC gain for a
	 * low-pass filter and the Nyquist gain for a high-pass filter. It is 0 by
	 * default.
	 * 
	 * <p>
	 * Using the base 2 exponent of the value contrains the possible gains to
	 ...
	 * identifies a subband within a resolution level and a decomposition level
	 * within it. Note that only leaf elements represent "real" subbands, while
	 * node elements represent only intermediate stages.
	 * 
	 * <p>
	 * It is defined recursively. The root node gets a value of 0. For a given
	 ...
	 * respect to the canvas origin, in the component's grid and subband's
	 * decomposition level. This is the real vertical index of the first column
	 ...
	 * the child subbands are calculated by taking into account the position of
	 * the subband in the canvas.
	 * 
	 * <p>
	 ...
	 * 
	 * @param ulcy
	 *            The vertical coordinate of the upper-left corner with respect
	 ...
	 * 
	 * @param hfilters
	 *            The horizontal wavelet filters (analysis or synthesis) for
	 *            each resolution level, starting at resolution level 0. If
	 *            there are less elements in the array than there are resolution
	 *            levels, the last element is used for the remaining resolution
	 *            levels.
	 * 
	 * @param vfilters
	 *            The vertical wavelet filters (analysis or synthesis) for each
	 *            resolution level, starting at resolution level 0. If there are
	 *            less elements in the array than there are resolution levels,
			...
			case WT_ORIENT_HH:
				...
				switch (sb.orientation)
				{
					case WT_ORIENT_LL:
						...
						if (sb == null || sb.resLvl != resLvl)
						{
							// Already at top-level or last subband in res.
							// level
							return null;
						}
						sb = sb.getHL();
	 ...
	 * belongs. The specified point must be inside this (i.e. the one defined by
	 * this object) subband. This method searches through the tree.
	 * 
	 * @param x
