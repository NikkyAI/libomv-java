 ...
 * describes the subband decomposition for a wavelet transform, specifically for
 * the analysis side.
 * 
 * <p>
...
public class SubbandAn extends Subband
{
	 ...
	 * if there was an error of exactly one quantization step size in the sample
	 * of the subband. This value is expressed relative to a nominal dynamic
	 * range in the image domain of exactly 1.0. This field contains valid data
	 ...
	 * 
	 * @param hfilters
	 *            The horizontal wavelet analysis filters for each resolution
	 *            level, starting at resolution level 0.
	 * 
	 * @param vfilters
	 *            The vertical wavelet analysis filters for each resolution
	 *            level, starting at resolution level 0.
	 * 
	 ...
	 *            It has to be a AnWTFilter object.
	 * 
	 * @param vfilter
	 *            The vertical wavelet filter used to decompose this subband. It
	 *            has to be a AnWTFilter object.
	 * 
	...
	/**
	 * Calculates the basis waveform of the first leaf for which the L2-norm has
	 * not been calculated yet. This method searches recursively for the first
	 * leaf for which the value has not been calculated yet, and then calculates
	 * the L2-norm on the return path.
	 * 
	 * <p>
	...
	/**
	 * Assigns the given L2-norm to the first leaf that does not have an L2-norm
	 * value yet (i.e. l2norm is negative). The search is done recursively and
	 * in the same order as that of the calcBasisWaveForms() method, so that
	 * this method is used to assigne the l2norm of the previously computed
	 * waveforms.
	 * 
	 * <p>
	 * This method can not be called on an element that ahs a non-negative value
