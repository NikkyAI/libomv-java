 ...
 * quantized or not. This class does not have the notion of components.
 * Therefore, it should be used for data from a single component. Subclasses
 * should implement the different types of storage (<tt>int</tt>, <tt>float</tt>
 * , etc.).
 * 
 * <p>
 ...
 * element (i.e. the top-left coefficient). The member variable 'scanw' gives
 * the width of the scan that is used to store the data, that can be different
 * from the width of the block. Element '(x,y)' of the code-block (i.e. '(0,0)'
 * is the top-left coefficient), will appear at position 'offset+y*scanw+x' in
 * the array of data.
 * </p>
 * 
 * <p>
...
public abstract class CBlkWTData
{
	...
	/**
	 * The number of magnitude bits in the integer representation. This is only
	 * used for quantized wavelet data.
	 */
	public int magbits;
	 ...
	 * order to get the real absolute value. This value is useful to obtain the
	 * complement of 2 representation of a coefficient that is currently using
	 * the sign-magnitude representation.
	 */
	public double convertFactor = 1.0;

	/**
	 * The quantization step size of the code-block. The value is updated by the
	 * quantizer module
	 */
	 ...
	 * array of the correct type explicitely and not through an <tt>Object</tt>.
	 * </p>
	 * 
