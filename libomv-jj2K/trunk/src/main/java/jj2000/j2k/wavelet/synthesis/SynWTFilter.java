 ...
 * one. See the SynWTFilterInt and SynWTFilterFloat classes. Implementations of
 ...
 * length of output signal is N, the low-pass input signal is of length N/2 if N
 * is even and N/2+1/2 if N is odd, while the high-pass input signal is of
 * length N/2 if N is even and N/2-1/2 if N is odd.</li>
 * 
 * <li>The normalization of the analysis filters is 1 for the DC gain and 2 for
 * the Nyquist gain (Type I normalization), for both reversible and
 * non-reversible filters. The normalization of the synthesis filters should
 * ensure prefect reconstruction according to this normalization of the analysis
 * wavelet filters.</li>
 * 
 * </ul>
 *
 * <p>
 * The synthetize method may seem very complicated, but is designed to minimize
...
public abstract class SynWTFilter implements WaveletFilter, Markers
{
	 ...
	 * 
	 * <p>
	 * The input low-pass (high-pass) signal resides in the lowSig array. The
	 * index of the first sample to filter (i.e. that will generate the first
	 ...
	 * signals in the same lowSig (highSig) array, and this is determined by the
	 * lowStep (highStep) argument. This means that the first sample of the
	 * low-pass (high-pass) input signal is lowSig[lowOff] (highSig[highOff]),
	 * the second is lowSig[lowOff+lowStep] (highSig[highOff+highStep]), the
	 * third is lowSig[lowOff+2*lowStep] (highSig[highOff+2*highStep]), and so
	 * on. Therefore if lowStep (highStep) is 1 there is no interleaving. This
	 * feature allows to filter columns of a 2-D signal, when it is stored in a
	 ...
	 *            must be of the correct type (e.g., it must be int[] if
	 ...
	 * 
	 * @param lowLen
	 *            This is the number of samples in the low-pass input signal to
	 *            filter.
	 * 
	 * @param lowStep
	 *            This is the step, or interleave factor, of the low-pass input
	 *            signal samples in the lowSig array. See above.
	 * 
	 * @param highSig
	 *            This is the array that contains the high-pass input signal. It
	 *            must be of the correct type (e.g., it must be int[] if
	 *            getDataType() returns TYPE_INT).
	 * 
	 * @param highOff
	 ...
	 * 
	 * @param highLen
	 *            This is the number of samples in the high-pass input signal to
	 *            filter.
	 * 
	 * @param highStep
	 *            This is the step, or interleave factor, of the high-pass input
	 *            signal samples in the highSig array. See above.
	 * 
	 * @param outSig
	 *            This is the array where the output signal is placed. It must
	 *            be of the same type as lowSig and it should be long enough to
	 *            contain the output signal.
	 * 
	 * @param outOff
	 *            This is the index in outSig of the element where to put the
	 *            first output sample.
	 * 
	 * @param outStep
	 *            This is the step, or interleave factor, of the output samples
