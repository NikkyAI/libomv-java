...
public interface ForwWT extends WaveletTransform, ForwWTDataProps
{
	 ...
	 * specified tile-component. The first element in the array is the filter
	 * used to obtain the lowest resolution (resolution level 0) subbands (i.e.
	 * lowest frequency LL subband), the second element is the one used to
	 * generate the resolution level 1 subbands, and so on. If there are less
	 * elements in the array than the number of resolution levels, then the last
	 * one is assumed to repeat itself.
	 * 
	 ...
	 * LL band, in the specified tile-component. A value of 0 means that no
