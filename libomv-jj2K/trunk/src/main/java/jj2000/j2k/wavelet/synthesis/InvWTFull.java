...
public class InvWTFull extends InverseWT
{
	...
	private boolean isSubbandReversible(Subband subband)
	{
		if (subband.isNode)
		{
			...
			return isSubbandReversible(subband.getLL()) && isSubbandReversible(subband.getHL())
					&& isSubbandReversible(subband.getLH()) && isSubbandReversible(subband.getHH())
	...
	public int setTile(int x, int y)
	{
		...
		{
			for (int i = reconstructedComps.length - 1; i >= 0; i--)
			{
