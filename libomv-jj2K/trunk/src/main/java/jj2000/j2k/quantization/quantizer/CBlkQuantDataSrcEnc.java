 ...
 * to transfer them in a code-block by code-block basis. In each call to
 * 'getNextCodeBlock()' or 'getNextInternCodeBlock()' a new code-block is
 ...
 * representation, the most significant bit is used for the sign (0 if positive,
 * 1 if negative) and the magnitude of the coefficient is stored in the next M
 * most significant bits. The rest of the bits (least significant bits) can
