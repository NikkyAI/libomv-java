...
public class StdDequantizer extends Dequantizer
{
	 ...
	 * 
	 * @param rb
	 *            The number of "range bits" (bitdepth) for each component (must
	 ...
	 * integral (i.e. non-fractional) bit, which is equivalent to the number of
	...
	public final DataBlk getInternCodeBlock(int c, int m, int n, SubbandSyn sb, DataBlk cblk)
	{
		...
		{ // Not reversible
			if (derived)
			{
				...
				step = params.nStep[0][0] * (1L << (rb[c] + sb.anGainExp + mrl - sb.level));
			}
			else
			{
				step = params.nStep[sb.resLvl][sb.sbandIdx] * (1L << (rb[c] + sb.anGainExp));
			}
			shiftBits = 31 - magBits;
