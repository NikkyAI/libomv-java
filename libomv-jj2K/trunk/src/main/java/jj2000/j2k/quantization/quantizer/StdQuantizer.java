...
/**
 * This class implements scalar quantization of integer or floating-point valued
 * source data. The source data is the wavelet transformed image data and the
 * output is the quantized wavelet coefficients represented in sign-magnitude
 * (see below).
 * 
 * <p>
...
public class StdQuantizer extends Quantizer
{
	...
	/**
	 * The 'CBlkWTDataFloat' object used to request data, used when quantizing
	 * floating-point data.
	 */
	...
	/**
	 * Initializes the source of wavelet transform coefficients. The constructor
	 * takes information on whether the quantizer is in reversible, derived or
	 * expounded mode. If the quantizer is reversible the value of 'derived' is
	 * ignored. If the source data is not integer (int) then the quantizer can
	 * not be reversible.
	 * 
	 * <p>
	 ...
	 * representation. The mantissa occupies the 11 least significant bits (bits
	 * 10-0), and the exponent the previous 5 bits (bits 15-11).
	 * 
	 * @param step
	...
	/**
	 * Returns the maximum number of magnitude bits in any subband in the given
	 * tile-component if derived quantization is used
	 * 
	 * @param sb
	...
	/**
	 * Returns the maximum number of magnitude bits in any subband in the given
	 * tile-component if expounded quantization is used
	 * 
	 * @param sb
	 *            The root of the subband tree of the tile-component
	 * 
	...
	private int getMaxMagBitsExpounded(Subband sb, int t, int c)
	{
		...
		if (!sb.isNode)
		{
			float baseStep = ((Float) qsss.getTileCompVal(t, c)).floatValue();
			return g - 1
					- (int) Math.floor(Math.log(baseStep / (((SubbandAn) sb).l2Norm * (1 << sb.anGainExp))) / log2);
		}
