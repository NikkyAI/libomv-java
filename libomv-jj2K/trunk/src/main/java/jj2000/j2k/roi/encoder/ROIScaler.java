 ...
 * 
 * <p>
 * The ROI method is the Maxshift method. The ROIScaler works by scaling the
 * quantized wavelet coefficients that do not affect the ROI (i.e background
 * coefficients) so that these samples get a lower significance than the ROI
...
public class ROIScaler extends ImgDataAdapter implements CBlkQuantDataSrcEnc
{
	...
	private final static String[][] pinfo = {
			{
					...
					"Specifies ROIs shape and location. The shape can be either "
							...
							+ "the size as the image. The component idx specifies which components "
							...
							+ "default unless it is overridden by the 'Aptype' option.", null },
			{
					"Ralign",
					"[on|off]",
							...
							+ "but by instead scaling the distortion estimates.", "off" },
			{
					...
					"This argument forces the lowest <level> resolution levels to "
							...
							+ "belongs to the ROI, 1 means the two lowest etc. (-1 deactivates the option)", "-1" },
			{
					"Rno_rect",
					"[on|off]",
					"This argument makes sure that the ROI mask generation is not done "
							+ "using the fast ROI mask generation for rectangular ROIs "
							+ "regardless of whether the specified ROIs are rectangular or not", "off" }, };
	...
	/**
	 * Calculates the maximum amount of magnitude bits for each tile-component,
	 * and stores it in the 'maxMagBits' array. This is called by the
	 * constructor
	 * 
	 * @param encSpec
	 *            The encoder specifications for addition of roi specs
