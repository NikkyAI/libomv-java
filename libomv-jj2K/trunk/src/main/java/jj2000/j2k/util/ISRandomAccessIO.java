...
public class ISRandomAccessIO implements RandomAccessIO
{
	...
	public long readUnsignedInt() throws IOException
	{
		...
		// general case
		return (0xFFFFFFFFL & ((read() << 24) | (read() << 16) | (read() << 8) | read()));
	}
	...
	public float readFloat() throws IOException
	{
		if (pos + 3 < len)
		{ // common, fast case
			return Float.intBitsToFloat((buf[pos++] << 24) | ((0xFF & buf[pos++]) << 16) | ((0xFF & buf[pos++]) << 8)
					| (0xFF & buf[pos++]));
		}
		// general case
	...
	public double readDouble() throws IOException
	{
		if (pos + 7 < len)
		{ // common, fast case
			return Double.longBitsToDouble(((long) buf[pos++] << 56) | ((long) (0xFF & buf[pos++]) << 48)
					| ((long) (0xFF & buf[pos++]) << 40) | ((long) (0xFF & buf[pos++]) << 32)
					| ((long) (0xFF & buf[pos++]) << 24) | ((long) (0xFF & buf[pos++]) << 16)
					| ((long) (0xFF & buf[pos++]) << 8) | (0xFF & buf[pos++]));
		}
		// general case
		return Double.longBitsToDouble(((long) read() << 56) | ((long) read() << 48) | ((long) read() << 40)
				| ((long) read() << 32) | ((long) read() << 24) | ((long) read() << 16) | ((long) read() << 8)
				| read());
	}
