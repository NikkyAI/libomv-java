...
/**
 * This class presents a collection of static methods that allow access to some
...
public final class NativeServices
{
	 ...
	 * Machine (JVM) to the specified level. The concurrency level specifies how
	 * many threads can run in parallel on different CPUs at any given time for
	 * JVM implementations that use POSIX threads with PTHREAD_SCOPE_PROCESS
	 * scheduling scope. A concurrency level of 0 means that the operating
	 * system will automatically adjust the concurrency level depending on the
	 * number of threads blocking on system calls, but this will probably not
	 * exploit more than one CPU in multiporocessor machines. If the concurrency
	...
	/**
	 * Calls the POSIX threads 'pthread_getconcurrency', or equivalent, function
	 ...
	 * and returns true on success. Multiple calls to this method after a
	 * successful call have no effect and return true. Multiple calls to this
	 * method after unsuccesful calls will make new attempts to load the
	 * library.
	 * 
	 ...
	 * if not yet loaded. If the library can not be found (either in a previous
	 * attempt to load it or in an attempt in this method) an
