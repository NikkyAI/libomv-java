 ...
 * 
 * <p>
 * If the Java Virtual Machine (JVM) uses native threads, then the different
 * threads will be able to execute in different processors in parallel on
 * multiprocessors machines. However, under some JVMs and operating systems
 * using native threads is not sufficient to allow the JVM access to multiple
 * processors. This is the case when native threads are implemented using POSIX
 * threads on lightweight processes (i.e. PTHREAD_SCOPE_PROCESS sopce
 * scheduling), which is the case on most UNIX operating systems. In order to do
 * provide access to multiple processors it is necessary to set the concurrency
 * level to the number of processors or slightly higher. This can be achieved by
 * setting the Java system property with the name defined by
 ...
 * Idle threads can be given a target which is run. While running the target the
 ...
 * any target's 'run()' method, or any method called from it, does not use
 ...
 * 'Error' and one for 'RuntimeException'). If a target's 'run()' method throws
 * an 'Error' or 'RuntimeException' the corresponding exception condition is set
 * and the exception object saved. In any subsequent call to
 ...
 * 'run()' method throws the 'ThreadDeath' exception a warning message is
 * printed and the exception is propagated, which will terminate the thread in
 * which it occurs. This could lead to instabilities of the pool. The
...
public class ThreadPool
{
	...
	/**
	 * The array of idle threads and the lock for the manipulation of the idle
	 * thread list.
	 */
	...
	class ThreadPoolThread extends Thread
	{
		...
		/**
		 * Creates a ThreadPoolThread object, setting its name according to the
		 * given 'idx', daemon type and the priority to the one of the pool.
		 * 
		 * @param idx
		 ...
		 * idle state in the pool and then enters an infinite loop. In this loop
		 * it waits until a target to run exists and runs it. Once the target's
		 * run() method is done it re-joins the idle state and notifies the
		 * waiting lock object, if one exists.
		 * 
		 * <p>
		 ...
		 * this thread is not affected, except for 'ThreadDeath'. If a
		 ...
		 * exceptions which are subclasses of 'Error' or 'RuntimeException' the
			...
			synchronized (this)
			{
				while (true)
				{
					...
					catch (Throwable ue)
					{
						...
						// never be.
						targetRE = new RuntimeException("Unchecked exception " + "thrown by target's "
								+ "run() method in pool " + poolName + ".");
					}
					// Join idle threads
					putInIdleList(this);
		 ...
		 * notify mode. The another target is currently running the method will
		 * block until it terminates. After setting the new target the runner
		 * thread will be wakenup and execytion will start.
		 * 
		 * @param target
		 *            The runnable object containing the 'run()' method to run.
		 * 
		 * @param lock
		 *            An object on which notify will be called once the target's
		 *            run method has finished. A thread to be notified should be
		 *            waiting on that object. If null no thread is notified.
		 * 
		 * @param notifyAll
		 *            If true 'notifyAll()', instead of 'notify()', will be
		 *            called on tghe lock.
		 */
	 ...
	 * 
	 * @param priority
	 *            The priority to give to the threads in the pool. If less than
			...
			else
			{
				...
				FacilityManager.getMsgLogger().printmsg(
						...
						"Native library to set " + "thread concurrency level as specified by the "
								+ CONCURRENCY_PROP_NAME + " property not found. " + "Thread concurrency unchanged.");
			}
		}
	 ...
	 * 
	 * @param notifyAll
	 *            If true, threads waiting on the lock of the 'l' object will be
	 ...
	 * 
	 * @exception Error
	 *                If an error condition has been thrown by a target 'run()'
	 *                method.
	 * 
	 * @exception RuntimeException
	 *                If a runtime exception has been thrown by a target 'run()'
	 *                method.
	 */
