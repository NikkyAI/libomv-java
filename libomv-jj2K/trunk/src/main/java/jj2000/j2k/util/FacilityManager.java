...
/**
 * This class manages common facilities for multi-threaded environments, It can
 * register different facilities for each thread, and also a default one, so
 * that they can be referred by static methods, while possibly having different
 * ones for different threads. Also a default facility exists that is used for
 * threads for which no particular facility has been registerd registered.
 * 
 * <p>
...
public class FacilityManager
{
	...
	private final static Hashtable<Thread, ProgressWatch> watchProgList = new Hashtable<Thread, ProgressWatch>();
	 ...
	 * If any other logging facility was registered with the thread 't' it is
	 * overriden by 'ml'. If 't' is null then 'ml' is taken as the default
	 * message logger that is used for threads that have no MsgLogger
	 * registered.
	 * 
	 * @param t
	...
	/**
	 * Returns the MsgLogger registered with the current thread (the thread that
	 * calls this method). If the current thread has no registered MsgLogger
	 * then the default message logger is returned.
	 * 
	...
	/**
	 * Returns the MsgLogger registered with the thread 't' (the thread that
	 * calls this method). If the thread 't' has no registered MsgLogger then
	 * the default message logger is returned.
	 * 
	 * @param t
