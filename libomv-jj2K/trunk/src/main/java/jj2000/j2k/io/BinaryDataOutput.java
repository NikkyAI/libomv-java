 ...
 * 
 * <P>
 * Byte level output (i.e., for byte, int, long, float, etc.) should always be
 * byte aligned. For example, a request to write an <tt>int</tt> should always
 * realign the output at the byte level.
 * 
 * <P>
 * The implementation of this interface should clearly define if multi-byte
 * output data is written in little- or big-endian byte ordering (least
...
public interface BinaryDataOutput
{
	...
	/**
	 * Should write the byte value of <tt>v</tt> (i.e., 8 least significant
	 * bits) to the output. Prior to writing, the output should be realigned at
	 * the byte level.
	 * 
	 * <P>
	...
	/**
	 * Should write the short value of <tt>v</tt> (i.e., 16 least significant
	 * bits) to the output. Prior to writing, the output should be realigned at
	 * the byte level.
	 * 
	 * <P>
	...
	/**
	 * Should write the int value of <tt>v</tt> (i.e., the 32 bits) to the
	...
	/**
	 * Should write the long value of <tt>v</tt> (i.e., the 64 bits) to the
	 * output. Prior to writing, the output should be realigned at the byte
	 * level.
	 * 
	 * @param v
	...
	/**
	 * Should write the IEEE float value <tt>v</tt> (i.e., 32 bits) to the
	 * output. Prior to writing, the output should be realigned at the byte
	 * level.
	 * 
	 * @param v
	...
	/**
	 * Should write the IEEE double value <tt>v</tt> (i.e., 64 bits) to the
	 * output. Prior to writing, the output should be realigned at the byte
	 * level.
	 * 
	 * @param v
	...
	/**
	 * Any data that has been buffered must be written, and the stream should be
