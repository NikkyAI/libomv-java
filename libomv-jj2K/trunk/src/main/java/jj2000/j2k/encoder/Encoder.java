...
public class Encoder extends ImgEncoder implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			...
			int fileLength = encode(imgsrc, refx, refy, trefx, trefy, tw, th, ncomp, ppminput, outname, bwriter, rate,
					                imsigned, useFileFormat, pphTile, pphMain, tempSop, tempEph, pktspertp);
			if (fileLength < 0)
				return;
