...
public class StdEntropyDecoder extends EntropyDecoder implements StdEntropyCoderOptions
{
	...
	/**
	 * Flag to indicate if we should be verbose about bit stream errors detected
	 * with the error resilience options
	 */
	...
	/**
	 * The 4 bits of the error resilience segmentation symbol (decimal 10, which
	 * is binary sequence 1010)
	 */
	 ...
	 * of a coefficient in row 'i' and column 'j', while the upper 16 bits store
	 * the state of a coefficient in row 'i+1' and column 'j'. The 'i' row is
	 * either the first or the third row of a stripe. This packing of the states
	 * into 32 bit words allows a faster scan of all coefficients on each coding
	 * pass and diminished the amount of data transferred. The size of the state
	 ...
	 *            'OPT_TERM_PASS', 'OPT_RESET_MQ', 'OPT_VERT_STR_CAUSAL',
	 *            'OPT_BYPASS' and 'OPT_SEG_SYMBOLS' option flags.
	 * 
	 * @param doer
	 ...
	 * 
	 * @param verber
	 *            This flag indicates if the entropy decoder should be verbose
	 *            about bit stream errors that are detected and concealed.
	 * 
	 * @param mQuit
	...
	public DataBlk getCodeBlock(int c, int m, int n, SubbandSyn sb, DataBlk cblk)
	{
		...
		if (curbp >= 0 && npasses > 0)
		{
			isterm = (options & OPT_TERM_PASS) != 0
					|| ((options & OPT_BYPASS) != 0 && (31 - NUM_NON_BYPASS_MS_BP - srcblk.skipMSBP) >= curbp);
		...
		if (!error || !doer)
		{
			while (curbp >= 0 && npasses > 0)
			{
				...
				if ((options & OPT_TERM_PASS) != 0
						|| ((options & OPT_BYPASS) != 0 && (curbp < 31 - NUM_NON_BYPASS_MS_BP - srcblk.skipMSBP)))
				{
					// Here starts a new MQ segment
					mq.nextSegment(null, -1, srcblk.tsLengths[++tsidx]);
				}
	 ...
	 * returns true if an error is detected, or false otherwise. If an error is
	 * detected it means that the bit stream contains some erroneous bit that
	 * have led to the decoding of incorrect data. This data affects the whole
	 ...
	 * This method also checks for segmentation markers if those are present and
	 * returns true if an error is detected, or false otherwise. If an error is
	 * detected it measn that the bit stream contains some erroneous bit that
	 * have led to the decoding of incorrect data. This data affects the whole
	 * last decoded bit-plane (i.e. 'bp'). If 'true' is returned the 'conceal'
	 * method should be called and no more passes should be decoded for this
	 * code-block's bit stream.
	...
	/**
	 * Performs the cleanup pass on the specified data and bit-plane. It decodes
	...
	/**
	 * Conceals decoding errors detected in the last bit-plane. The concealement
	 * resets the state of the decoded data to what it was before the decoding
	 * of bit-plane 'bp' started. No more data should be decoded after this
	 * method is called for this code-block's data to which it is applied.
	 * 
	 * @param cblk
