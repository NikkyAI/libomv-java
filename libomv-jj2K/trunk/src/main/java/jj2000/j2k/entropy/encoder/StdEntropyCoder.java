 ...
 * "jj2000.j2k.entropy.encoder.StdEntropyCoder.nthreads" Java system property.
...
public class StdEntropyCoder extends EntropyCoder
{
	 ...
	 */
	private static final int SEG_SYMB_CTXTS[] = { UNIF_CTXT, UNIF_CTXT, UNIF_CTXT, UNIF_CTXT };
...
//			| STATE_NZ_CTXT_R1 | STATE_NZ_CTXT_R2;
	...
	private class Compressor implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				compressCodeBlock(c, ccb, srcblkT[idx], mqT[idx], boutT[idx], outT[idx], stateT[idx], distbufT[idx],
						ratebufT[idx], istermbufT[idx], symbufT[idx], ctxtbufT[idx], options, rev, lcType, tType);
				if (DO_TIMING)
			...
			StringSpec mqrs, StringSpec rts, StringSpec css, StringSpec sss, StringSpec lcs, StringSpec tts)
	{
		...
		if (nt > 0)
		{
			...
			tsl = nt;
			tPool = new ThreadPool(nt, Thread.currentThread().getPriority() + THREADS_PRIORITY_INC, "StdEntropyCoder");
			idleComps = new Stack<Compressor>();
			...
			{
				completedComps[i] = new Stack<Compressor>();
			}
			for (i = 0; i < nt; i++)
			{
	...
	public void finalize() throws Throwable
	{
		if (DO_TIMING)
		{
			...
			{ // Multithreaded implementation
				...
				Enumeration<Compressor> e = idleComps.elements();
	 ...
	 * 
	 * @param distbuf
	 *            The buffer where to store the distortion at the end of each
	 *            coding pass.
	 * 
	 * @param ratebuf
	 *            The buffer where to store the rate (i.e. coded lenth) at the
	 *            end of each coding pass.
	 * 
	 * @param istermbuf
	 *            The buffer where to store the terminated flag for each coding
	 *            pass.
	 * 
	 * @param symbuf
	 ...
	 * 
	 * @param ctxtbuf
	 *            A buffer to hold the contexts to use in sending the buffered
			...
			boolean istermbuf[], int symbuf[], int ctxtbuf[], int options, boolean rev, int lcType, int tType)
	{
		...
		if (curbp >= lmb)
		{
			...
			istermbuf[npass] = (options & OPT_TERM_PASS) != 0 || curbp == lmb
					|| ((options & OPT_BYPASS) != 0 && (31 - NUM_NON_BYPASS_MS_BP - skipbp) >= curbp);
		...
		while (curbp >= lmb)
		{
			...
			if ((options & OPT_BYPASS) == 0 || (31 - NUM_NON_BYPASS_MS_BP - skipbp <= curbp))
			{ // No bypass coding
				totdist += sigProgPass(srcblk, mq, istermbuf[npass], curbp, state, fs, zc_lut, symbuf, ctxtbuf,
						ratebuf, npass, ltpidx, options) * msew;
			}
			else
			{ // Bypass ("raw") coding
		...
		ccb.selectConvexHull(ratebuf, distbuf, (options & (OPT_BYPASS | OPT_TERM_PASS)) != 0 ? istermbuf : null, npass,
				rev);
	 ...
	 * 
	 * @param doterm
	 *            If true it performs an MQ-coder termination after the end of
	 *            the pass
	 * 
	 * @param bp
	 ...
	 * 
	 * @param ltpidx
	 *            The index of the last pass that was terminated, or negative if
	 *            none.
	 * 
	 * @param options
	 ...
	 * 
	 * @param doterm
	 *            If true the bit based output is byte aligned after the end of
	 *            the pass.
	 * 
	 * @param bp
	 *            The bit-plane to code
...
//						nsym++;
	 ...
	 * 
	 * @param rates
	 *            The rate (i.e. accumulated number of bytes) for each coding
	 *            pass
	 * 
	 * @param isterm
