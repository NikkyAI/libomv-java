...
public class EBCOTRateAllocator extends PostCompRateAllocator
{
	...
	public void initialize() throws IOException
	{
		...
		for (int t = 0; t < numTiles; t++)
		{
			if (encSpec.pocs.isTileSpecified(t))
			{
				...
				if (nValidProg == 0)
				{
					throw new Error("Unable to initialize rate allocator: No default progression type has been "
							+ "defined for tile " + t);
				}
			}
		} // End loop on tiles
	...
	private void buildAndWriteLayers() throws IOException
	{
		...
		{ // loop on layers
			...
			else
			{
				...
				{
					throw new IllegalArgumentException("The first and the last layer thresholds"
							+ " must be optimized");
				}
			...
			{ // loop on tiles
				...
				{ // loop on components
					...
					sopUsed = ((String) encSpec.sops.getTileDef(t)).equalsIgnoreCase("on");
					// set boolean ephUsed here (EPH markers)
					ephUsed = ((String) encSpec.ephs.getTileDef(t)).equalsIgnoreCase("on");
					...
					{ // loop on resolution levels
						...
						{ // loop on precincts
							...
							{
								tmp = bsWriter.writePacketHead(hBuff.getBuffer(), hBuff.getLength(), true, sopUsed,
										ephUsed);
								tmp += bsWriter.writePacketBody(pktEnc.getLastBodyBuf(), pktEnc.getLastBodyLen(), true,
										pktEnc.isROIinPkt(), pktEnc.getROILen());
								actualBytes += tmp;
								tileLengths[t] += tmp;
							}
			...
			{ // loop on layers
				...
				{// loop on components
					...
					{ // loop on precincts
						...
						if (pktEnc.isPacketWritable())
						{
							bsWriter.writePacketHead(hBuff.getBuffer(), hBuff.getLength(), false, sopUsed, ephUsed);
							bsWriter.writePacketBody(pktEnc.getLastBodyBuf(), pktEnc.getLastBodyLen(), false,
									pktEnc.isROIinPkt(), pktEnc.getROILen());
						}
	 ...
	 * 
	 * @param maxBytes
	 *            The maximum number of bytes that can be written. It includes
					...
					for (int r = 0; r < numLvls; r++)
					{
						...
						for (int p = 0; p < nPrec; p++)
						{
							...
							if (pktEnc.isPacketWritable())
							{
								...
								actualBytes += bsWriter.writePacketHead(hBuff.getBuffer(), hBuff.getLength(), true,
										sopUsed, ephUsed);
								actualBytes += bsWriter.writePacketBody(bBuff, pktEnc.getLastBodyLen(), true,
										pktEnc.isROIinPkt(), pktEnc.getROILen());
							}
						} // end loop on precincts
						sb = sb.parent;
	 ...
	 * 
	 * @param subb
	 *            The LL subband in the resolution level lvlIdx, which is parent
