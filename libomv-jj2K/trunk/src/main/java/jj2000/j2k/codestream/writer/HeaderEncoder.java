...
public class HeaderEncoder implements Markers, StdEntropyCoderOptions
{
	...
	protected void writeCOD(boolean mh, int tileIdx) throws IOException
	{
		...
		if (ppx != PRECINCT_PARTITION_DEF_SIZE || ppy != PRECINCT_PARTITION_DEF_SIZE)
		{
		...
		if (mh)
		{
			if ((encSpec.sops.getDefault().toString()).equalsIgnoreCase("on"))
			{
				tmp |= SCOX_USE_SOP;
			}
		}
		...
		{
			if ((encSpec.sops.getTileDef(tileIdx).toString()).equalsIgnoreCase("on"))
			{
				tmp |= SCOX_USE_SOP;
			}
		}
		...
		if (mh)
		{
			if ((encSpec.ephs.getDefault().toString()).equalsIgnoreCase("on"))
			{
				tmp |= SCOX_USE_EPH;
			}
		}
		...
		{
			if ((encSpec.ephs.getTileDef(tileIdx).toString()).equalsIgnoreCase("on"))
			{
				tmp |= SCOX_USE_EPH;
			}
		}
		...
		if (precinctPartitionUsed)
		{
			...
			if (mh)
			{
				v = (Vector<Integer>[]) encSpec.pss.getDefault();
			}
			else
			{
				v = (Vector<Integer>[]) encSpec.pss.getTileDef(tileIdx);
			}
			for (int r = mrl; r >= 0; r--)
			{
				if (r >= v[1].size())
...
//			prog = (Progression[]) (encSpec.pocs.getCompDef(compIdx));
		...
		{
			mrl = ((Integer) encSpec.dls.getTileCompVal(tileIdx, compIdx)).intValue();
...
//			prog = (Progression[]) (encSpec.pocs.getTileCompVal(tileIdx, compIdx));
		...
		{ // Main header
			...
			// MQ reset after each coding pass ?
			if (((String) encSpec.mqrs.getCompDef(compIdx)).equalsIgnoreCase("on"))
			{
				tmp |= OPT_RESET_MQ;
			}
			// MQ termination after each arithmetically coded coding pass ?
		...
		if (precinctPartitionUsed)
		{
			...
			if (mh)
			{
				v = (Vector<Integer>[]) encSpec.pss.getCompDef(compIdx);
			}
			else
			{
				v = (Vector<Integer>[]) encSpec.pss.getTileCompVal(tileIdx, compIdx);
			}
			for (int r = mrl; r >= 0; r--)
			{
				if (r >= v[1].size())
	...
	protected void writeTileQCC(int t, int compIdx) throws IOException
	{
		...
		float baseStep = ((Float) encSpec.qsss.getTileCompVal(t, compIdx)).floatValue();
	...
	public void encodeMainHeader() throws IOException
	{
		...
		// +-------------------------------+
		boolean isEresUsed = ((String) encSpec.tts.getDefault()).equals("predict");
		writeCOD(true, 0);
		...
		for (i = 0; i < nComp; i++)
		{
			boolean isEresUsedinComp = ((String) encSpec.tts.getCompDef(i)).equals("predict");
			if (encSpec.wfs.isCompSpecified(i) || encSpec.dls.isCompSpecified(i) || encSpec.bms.isCompSpecified(i)
					|| encSpec.mqrs.isCompSpecified(i) || encSpec.rts.isCompSpecified(i)
					|| encSpec.sss.isCompSpecified(i) || encSpec.css.isCompSpecified(i)
					|| encSpec.pss.isCompSpecified(i) || encSpec.cblks.isCompSpecified(i)
		...
		for (i = 0; i < nComp; i++)
		{
			if (dwt.getNomRangeBits(i) != defimgn || encSpec.qts.isCompSpecified(i) || encSpec.qsss.isCompSpecified(i)
					|| encSpec.dls.isCompSpecified(i) || encSpec.gbs.isCompSpecified(i))
			{
				writeMainQCC(i);
			}
		}
	...
	private void writeRGN(int tIdx) throws IOException
	{
		...
		for (i = 0; i < nComp; i++)
		{
			...
			hbuf.writeByte(((Integer) (encSpec.rois.getTileCompVal(tIdx, i))).intValue());
		}
	}
	...
	public void encodeTilePartHeader(int tileLength, int tileIdx) throws IOException
	{
		...
		// +--------------------------+
		boolean isEresUsed = ((String) encSpec.tts.getDefault()).equals("predict");
		boolean isEresUsedInTile = ((String) encSpec.tts.getTileDef(tileIdx)).equals("predict");
		...
		for (int c = 0; c < nComp; c++)
		{
			boolean isEresUsedInTileComp = ((String) encSpec.tts.getTileCompVal(tileIdx, c)).equals("predict");
			...
			{
				if (encSpec.wfs.isCompSpecified(c) || encSpec.dls.isCompSpecified(c) || encSpec.bms.isCompSpecified(c)
						|| encSpec.mqrs.isCompSpecified(c) || encSpec.rts.isCompSpecified(c)
						|| encSpec.sss.isCompSpecified(c) || encSpec.css.isCompSpecified(c)
						|| encSpec.pss.isCompSpecified(c) || encSpec.cblks.isCompSpecified(c)
						|| (encSpec.tts.isCompSpecified(c) && ((String) encSpec.tts.getCompDef(c)).equals("predict")))
				{
					writeCOC(false, tileIdx, c);
				}
			}
		...
		for (int c = 0; c < nComp; c++)
		{
			...
			{
				if (encSpec.qts.isCompSpecified(c) || encSpec.qsss.isCompSpecified(c) || encSpec.dls.isCompSpecified(c)
