 ...
 * head and a body. The codestream always has a maximum number of bytes that can
 * be written to it. After that many number of bytes no more data is written to
 * the codestream but the number of bytes is counted so that the value returned
 * by getMaxAvailableBytes() is negative. If the number of bytes is unlimited a
...
public abstract class CodestreamWriter
{
	 ...
	 */
	public abstract int writePacketHead(byte head[], int hlen, boolean sim, boolean sop, boolean eph)
			throws IOException;
	 ...
	 * @see #commitBitstreamHeader
	 */
	public abstract int writePacketBody(byte body[], int blen, boolean sim, boolean roiInPkt, int roiLen)
			throws IOException;
