...
/**
 * This class implements a CodestreamWriter for Java streams. The streams can be
 * files or network connections, or any other resource that presents itself as a
 * OutputStream. See the CodestreamWriter abstract class for more details on the
 * implementation of the CodestreamWriter abstract class.
 * 
 * <p>
 * Before any packet data is written to the bit stream (even in simulation mode)
 * the complete header should be written otherwise incorrect estimates are given
 * by getMaxAvailableBytes() for rate allocation.
 * 
...
public class FileCodestreamWriter extends CodestreamWriter implements Markers
{
	 ...
	 * header encoder. The magic number is written to the bit stream. Normally,
	 * the header encoder must be empty (i.e. no data has been written to it
	 * yet). No BufferedOutputStream is used on top of the output stream 'os'.
	 * 
	 * @param os
	 ...
	 *                If an error occurs while writing the magic number to the
	 *                'os' output stream.
	 */
	 ...
	 * used by this header. It returns the total number of bytes that the packet
	 * head takes in the bit stream. If in simulation mode then no data is
	 * written to the bit stream but the number of bytes is calculated. This can
	 * be used for iterative rate allocation.
	 * 
	 * <P>
	 * If the length of the data that is to be written to the bit stream is more
	 * than the space left (as returned by getMaxAvailableBytes()) only the data
	 * that does not exceed the allowed length is written, the rest is
	 * discarded. However the value returned by the method is the total length
	 * of the packet, as if all of it was written to the bit stream.
	 * 
	 * <P>
	 * If the bit stream header has not been commited yet and 'sim' is false,
	 ...
	 * that does not exceed the allowed length is written, the rest is
	 * discarded. However the value returned by the method is the total length
	 * of the packet body , as if all of it was written to the bit stream.
	 * 
	 * @param body
