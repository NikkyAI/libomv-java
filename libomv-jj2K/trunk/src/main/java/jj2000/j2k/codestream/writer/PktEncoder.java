...
public class PktEncoder
{
	...
	/**
	 * The tag tree for inclusion information. The indexes are outlined below.
	 * Note that the layer indexes start at 1, therefore, the layer index minus
	 ...
	 * (referred as Lblock in the JPEG 2000 standard). The indexes are outlined
	 * below. Note that the layer indexes start at 1, therefore, the layer index
	 ...
	 * that no information has been included for the block, yet. The indexes are
	 * outlined below. The subband indices are used as they are defined in the
	 * Subband class. The tile indices start at 0 and follow a lexicographical
...
//		int maxPrec = ppinfo[t][c][r].length;
	 ...
	 * header. The code-blocks appear in a 3D array of CBlkRateDistStats, 'cbs'.
	 * The first index is the tile index in lexicographical order, the second
	 * index is the subband index (as defined in the Subband class), and the
	 * third index is the code-block index (whithin the subband tile) in
	 * lexicographical order as well. The indexes of the new truncation points
	 * for each code-block are specified by the 3D array of int 'tIndx'. The
	 * indices of this array are the same as for cbs. The truncation point
	 * indices in 'tIndx' are the indices of the elements of the 'truncIdxs'
	 * array, of the CBlkRateDistStats class, that give the real truncation
	 * points. If a truncation point index is negative it means that the
	 * code-block has not been included in any layer yet. If the truncation
	 * point is less than or equal to the highest truncation point used in
	 * previous layers then the code-block is not included in the packet.
					...
					if (cur_tIndx[b] > cur_prevtIdxs[b])
					{
						...
						else
						{
							...
							System.arraycopy(cur_cbs[b].data,
									cur_cbs[b].truncRates[cur_cbs[b].truncIdxs[cur_prevtIdxs[b]]], lbbuf, lblen, cblen);
						}
						lblen += cblen;
						...
						if (cur_cbs[b].nROIcoeff != 0
								&& (cur_prevtIdxs[b] == -1 || cur_cbs[b].truncIdxs[cur_prevtIdxs[b]] <= cur_cbs[b].nROIcp - 1))
						{
							roiInPkt = true;
							roiLen = lblen;
						}
	...
	/**
	 * Returns the buffer of the body of the last encoded packet. The length of
