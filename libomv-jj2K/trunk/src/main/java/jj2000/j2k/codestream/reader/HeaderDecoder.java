...
public class HeaderDecoder implements ProgressionType, Markers, StdEntropyCoderOptions
{
	 ...
	 * 
	 * @param comIdx
	 *            Occurence of this COM marker in eith main or tile-part header
	 * 
	 * @exception IOException
	 *                If an I/O error occurs while reading from the encoded
	 *                header stream
	...
	private void readQCD(DataInputStream ehs, boolean mainh, int tileIdx, int tpIdx) throws IOException
	{
		...
		{
			int maxrl = (mainh ? ((Integer) decSpec.dls.getDefault()).intValue() : ((Integer) decSpec.dls
					.getTileDef(tileIdx)).intValue());
			int j, rl;
			int minb, maxb, hpd;
			int tmp;
		...
		{
			int maxrl = (qType == SQCX_SCALAR_DERIVED) ? 0 : (mainh ? ((Integer) decSpec.dls.getDefault()).intValue()
	...
	private void readQCC(DataInputStream ehs, boolean mainh, int tileIdx, int tpIdx) throws IOException
	{
		...
		if (qType == SQCX_NO_QUANTIZATION)
		{
			int maxrl = (mainh ? ((Integer) decSpec.dls.getCompDef(cComp)).intValue() : ((Integer) decSpec.dls
					.getTileCompVal(tileIdx, cComp)).intValue());
			int j, rl;
			int minb, maxb, hpd;
		...
		{
			int maxrl = (qType == SQCX_SCALAR_DERIVED) ? 0 : (mainh ? ((Integer) decSpec.dls.getCompDef(cComp))
					.intValue() : ((Integer) decSpec.dls.getTileCompVal(tileIdx, cComp)).intValue());
			int j, rl;
			int minb, maxb, hpd;
	...
	private void readCOC(DataInputStream ehs, boolean mainh, int tileIdx, int tpIdx) throws IOException
	{
		...
		else
		{
			...
			decSpec.ecopts.setTileCompVal(tileIdx, cComp, new Integer(ecOptions));
		}
	}
	...
	private void readPPM(DataInputStream ehs) throws IOException
	{
		...
		if (pPMMarkerData == null)
		{
			...
			tileOfTileParts = new Vector<Integer>();
	...
	private void readFoundMainMarkSeg() throws IOException
	{
		...
		if ((nfMarkSeg & SIZ_FOUND) != 0)
		{
			bais = new ByteArrayInputStream((ht.get("SIZ")));
		...
		if ((nfMarkSeg & COM_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("COM" + i)));
		...
		if ((nfMarkSeg & CRG_FOUND) != 0)
		{
			bais = new ByteArrayInputStream((ht.get("CRG")));
		...
		if ((nfMarkSeg & COD_FOUND) != 0)
		{
			bais = new ByteArrayInputStream((ht.get("COD")));
		...
		if ((nfMarkSeg & COC_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("COC" + i)));
		...
		if ((nfMarkSeg & RGN_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("RGN" + i)));
		...
		if ((nfMarkSeg & QCD_FOUND) != 0)
		{
			bais = new ByteArrayInputStream((ht.get("QCD")));
		...
		if ((nfMarkSeg & QCC_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("QCC" + i)));
		...
		if ((nfMarkSeg & POC_FOUND) != 0)
		{
			bais = new ByteArrayInputStream((ht.get("POC")));
		...
		if ((nfMarkSeg & PPM_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("PPM" + i)));
	...
	public void readFoundTilePartMarkSeg(int tileIdx, int tpIdx) throws IOException
	{
		...
		if ((nfMarkSeg & PPT_FOUND) != 0)
		{
			...
			{
				bais = new ByteArrayInputStream((ht.get("PPT" + i)));
	 ...
	 *                If the end of the encoded header stream is reached before
	 ...
	 * channels to the channel definition for the appropriate colorspace.
	 * 
	 * @param src
	 *            The bit stream reader agent where to get code-block data from.
	 * @param csMap
	...
	public ByteArrayInputStream getPackedPktHead(int tile) throws IOException
	{
		...
		if (pkdPktHeaders == null)
		{
			...
			if (nPPMMarkSeg != 0)
			{
				...
				{
					t = (tileOfTileParts.elementAt(i)).intValue();
					// get Nppm value
