		...
		StdEntropyCoderOptions
{
			...
			boolean cdstrInfo, HeaderInfo hi) throws IOException
	{
		...
		if (rateInBytes)
		{
			trate = tnbytes * 8f / hd.getMaxCompImgWidth() / hd.getMaxCompImgHeight();
		}
		else
		{
			tnbytes = (int) (trate * hd.getMaxCompImgWidth() * hd.getMaxCompImgHeight()) / 8;
		}
		...
		try
		{
			while (remainingTileParts != 0)
			{
				...
				strInfo += "Tile-part " + tp + " of tile " + t + " : " + tilePartStart + ", " + tilePartLen[t][tp]
				...
				{ // loop on components
					...
					{ // loop on precincts
						...
						if (start > lastByte && curTilePart < firstPackOff[t].length - 1)
						{
							...
							lastByte = in.getPos() + tilePartLen[t][curTilePart] - 1 - tilePartHeadLen[t][curTilePart];
						}
					...
					{ // Components
						...
						{ // layers
							...
							if (start > lastByte && curTilePart < firstPackOff[t].length - 1)
							{
								curTilePart++;
								in.seek(firstPackOff[t][curTilePart]);
								lastByte = in.getPos() + tilePartLen[t][curTilePart] - 1
...
//		int numLayers = ((Integer) decSpec.nls.getTileDef(t)).intValue();
	...
	private void readTilePkts(int t) throws IOException
	{
		...
		else if (nBytes[t] < (totTileLen[t] - totTileHeadLen[t]))
		{
			...
			{
				pktHeadLen[i] = pktHL.elementAt(i).intValue();
			}
						...
						for (int m = 0; m < mnby; m++)
						{
							...
							for (int n = 0; n < mnbx; n++)
							{
								...
								for (int c = 0; c < nc; c++)
								{
									...
									if (cbI[c] == null || cbI[c][r] == null || cbI[c][r][s] == null
											|| cbI[c][r][s][m] == null || cbI[c][r][s][m][n] == null)
									{
										continue;
									}
									cb = cbI[c][r][s][m][n];
	...
	public int setTile(int x, int y)
	{
		...
		for (int c = 0; c < nc; c++)
		{
			...
			subbTrees[c] = new SubbandSyn(getTileCompWidth(t, c, mdl[c]), getTileCompHeight(t, c, mdl[c]), getResULX(c,
					mdl[c]), getResULY(c, mdl[c]), mdl[c], decSpec.wfs.getHFilters(t, c), decSpec.wfs.getVFilters(t, c));
	...
	public int nextTile()
	{
		...
		{ // If not at end of current tile line
			tIdx = setTile(ctX + 1, ctY);
		}
		else
		{ // Go to first tile at next line
			tIdx = setTile(0, ctY + 1);
		}
		return tIdx;
	}
