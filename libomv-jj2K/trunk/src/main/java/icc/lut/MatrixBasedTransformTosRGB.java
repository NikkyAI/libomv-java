...
public class MatrixBasedTransformTosRGB
{
	...
	private float[][] fBuf = null; // Intermediate output of the first LUT
									// operation.
	...
	public String toString()
	{
		...
		body.append(eol).append("dwMaxValues= ").append(String.valueOf(dwMaxValue[0])).append(", ")
				.append(String.valueOf(dwMaxValue[1])).append(", ").append(String.valueOf(dwMaxValue[2]));

		body.append(eol).append("dwShiftValues= ").append(String.valueOf(dwShiftValue[0])).append(", ")
				.append(String.valueOf(dwShiftValue[1])).append(", ").append(String.valueOf(dwShiftValue[2]));

		body.append(eol).append(eol).append("fLut= ").append(eol)
				.append(ColorSpace.indent("  ", "fLut[RED]=  " + fLut[0].toString())).append(eol)
				.append(ColorSpace.indent("  ", "fLut[GRN]=  " + fLut[1].toString())).append(eol)
				.append(ColorSpace.indent("  ", "fLut[BLU]=  " + fLut[2].toString()));
	...
	public MatrixBasedTransformTosRGB(RestrictedICCProfile ricc, int dwMaxValue[], int dwShiftValue[])
	{
		...
		matrix = createMatrix(ricc, dwMaxValue); // Create and matrix from the
													// ICC profile.
	...
	public void apply(DataBlkFloat inb[], DataBlkFloat outb[]) throws MatrixBasedTransformException
	{
		...
		for (int y = 0; y < inb[0].h; ++y)
		{
			...
			while (index < end)
			{
				...
				// final LUT.
				val = (int) (matrix[M00] * fBuf[RED][index] + matrix[M01] * fBuf[GREEN][index] + matrix[M02]
						* fBuf[BLUE][index] + 0.5);
				// Clip the calculated value if necessary..
				if (val < 0)
					out[0][index] = lut32[0];
				...
				val = (int) (matrix[M10] * fBuf[RED][index] + matrix[M11] * fBuf[GREEN][index] + matrix[M12]
						* fBuf[BLUE][index] + 0.5);
				// Clip the calculated value if necessary..
				if (val < 0)
					out[1][index] = lut32[0];
				...
				val = (int) (matrix[M20] * fBuf[RED][index] + matrix[M21] * fBuf[GREEN][index] + matrix[M22]
						* fBuf[BLUE][index] + 0.5);
				// Clip the calculated value if necessary..
				if (val < 0)
					out[2][index] = lut32[0];
	...
	private static void standardizeMatrixLineThroughLut(DataBlkInt inb, // input
																		// datablock
	...
	)
	{
		...
		for (int y = inb.uly; y < inb.uly + inb.h; ++y)
		{
			...
			{
				int i = inb.offset + (y - inb.uly) * inb.scanw + (x - inb.ulx); // pixel
																				// index.
	...
	private static void standardizeMatrixLineThroughLut(DataBlkFloat inb, // input
																			// datablock
			float[] out, // output data reference
