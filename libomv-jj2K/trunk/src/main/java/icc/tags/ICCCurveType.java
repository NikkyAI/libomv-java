...
public class ICCCurveType extends ICCTag
{
	...
	{
		StringBuffer rep = new StringBuffer("[").append(super.toString()).append(" nentries = ")
				.append(String.valueOf(nEntries)).append(", length = " + String.valueOf(entry.length) + " ... ");
	...
	protected ICCCurveType(int signature, byte[] data, int offset, int length)
	{
		...
		for (int i = 0; i < nEntries; ++i)
			entry[i] = ICCProfile.getShort(data, offset + 3 * ICCProfile.int_size + i * ICCProfile.short_size) & 0xFFFF;
	}
