...
public final class ChannelDefinitionBox extends JP2Box
{
	...
	private int ndefs;
	private Hashtable<Integer, int[]> definitions = new Hashtable<Integer, int[]>();
	...
	{
		Enumeration<Integer> keys = definitions.keys();
		while (keys.hasMoreElements())
		{
			int[] bfr = definitions.get(keys.nextElement());
			if (asoc == getAsoc(bfr))
				return getCn(bfr);
		}
		return asoc;
	...
	{
		int[] bfr = definitions.get(new Integer(channel));
		return getTyp(bfr);
	}
	...
	public String toString()
	{
		...
		{
			int[] bfr = definitions.get(keys.nextElement());
			rep.append(eol).append("  ").append("Cn= ").append(String.valueOf(getCn(bfr))).append(", ").append("Typ= ")
					.append(String.valueOf(getTyp(bfr))).append(", ").append("Asoc= ")
					.append(String.valueOf(getAsoc(bfr)));
		}
