...
/*	public static OSD parse(InputStream json)
	...
	public static OSD parse(JSONObject json)
	{
		if (json == null)
		{
			return new OSD();
		...
		switch (json.GetJsonType())
		{
			case JsonType.Boolean:
				return OSD.FromBoolean((boolean)json);
			case JsonType.Int:
			...
			case JsonType.Array:
				OSDArray array = new OSDArray(json.getCount());
			...
			case JsonType.Object:
				OSDMap map = new OSDMap(json.getCount());
				IDictionaryEnumerator e = ((IOrderedDictionary)json).iterator();
				while (e.MoveNext())
				{
					map.add((String)e.getKey(), DeserializeJson((JsonData)e.getValue()));
				}
				return map;
			case JsonType.None:
			default:
	...
	public static String serializeToString(OSD osd)
	{
		return serialize(osd).ToJson();
	}
	...
	public static JSONObject serialize(OSD osd)
	{
		switch (osd.getType())
		{
			case Boolean:
				return new JSONObject(osd.AsBoolean());
			case Integer:
				return new JSONObject(osd.AsInteger());
			case Real:
				return new JSONObject(osd.AsReal());
			case String:
			case Date:
			case URI:
			case UUID:
				return new JSONObject(osd.AsString());
			case Binary:
				...
				{
					jsonbinarray.add(new JSONObject(binary[i]));
				}
				return jsonbinarray;
			case Array:
				...
				{
					jsonarray.add(serialize(array.get(i)));
				}
				return jsonarray;
			case Map:
				...
				for (Entry<String, OSD> kvp : map.entrySet())
				{
					...
					JSONObject data = serializeNoDefaults(kvp.getValue());
					if (data != null)
					{
						jsonmap.put(kvp.getKey(), data);
					}
				}
				return jsonmap;
			case Unknown:
	...
	private static JSONObject serializeNoDefaults(OSD osd)
	{
		switch (osd.getType())
		{
			case Boolean:
				boolean b = osd.AsBoolean();
				if (!b)
				{
					return null;
				}
			...
			case UUID:
				...
				return new JSONObject(uuid.toString());
			case Binary:
				byte[] binary = osd.AsBinary();
				if (binary == Helpers.EmptyBytes)
				{
					return null;
				}
