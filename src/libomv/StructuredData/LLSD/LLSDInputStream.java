...
public class LLSDInputStream extends FilterInputStream
{
	 ...
	 * When the buffer is empty, <code>pos</code> is equal to <code>buf.length</code>;
	 * when the buffer is full, <code>pos</code> is equal to zero.
	 */
	private int pos;
    ...
    /**
     * Pushes a subset of the bytes in {@code buffer} back to this stream.
    ...
    /**
     * Pushes all the byte in {@code buffer} back to this reader. The bytes
     * are pushed back in such a way that the next character read from this
     * stream is buffer[0], then buffer[1] and so on.
     * <p>
